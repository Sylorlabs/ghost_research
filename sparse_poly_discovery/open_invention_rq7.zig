//! RESEARCH Q7 — E11 scale: 100 LLM-proposed features, engine sole judge.
//!
//! The LLM (author of rq7_proposals.json) proposes diverse grid features as JSON
//! structs {name, formula_kind, params}. This harness certifies held-out test accuracy ≥0.90
//! on five target families: parity, hidden_pair, sum_mod, oriented, monomial_sign.
//!
//! Novelty gate: Pearson |ρ|≥0.995 against Walsh χ_S, spectral cos/sin(ω·count),
//! centered monomial φ_mask, or world-pool divisibility (sum%mod_p, sign%mod_p).
//!
//! Pass bar: ≥2 non-equivalent (novel) promoted features on ≥2 distinct families.
//!
//! Run: zig build open-invention-rq7 --release=fast
//!      zig build open-invention-rq7 --release=fast -- rq7_proposals.json
...
const Family = enum { walsh, spectral, monomial, world, novel, @"unknown" };
const family_name = [_][]const u8{ "walsh", "spectral", "monomial", "world", "novel", "unknown" };
...
fn classifyFamily(
    ...
    for (WORLD_POOL) |pmod| {
        for (lo..hi) |s| {
            scratch[s] = if (gridSum(grid[s]) % pmod == 0) @as(f64, 1) else 0;
        }
        if (@abs(pearson(feat, scratch, lo, hi)) >= EQUIV) return .world;
        for (lo..hi) |s| {
            scratch[s] = @as(f64, @floatFromInt(gridSum(grid[s]) % pmod));
        }
        if (@abs(pearson(feat, scratch, lo, hi)) >= EQUIV) return .world;
        for (lo..hi) |s| {
            scratch[s] = if (@as(usize, signPattern(grid[s])) % pmod == 0) @as(f64, 1) else 0;
        }
        if (@abs(pearson(feat, scratch, lo, hi)) >= EQUIV) return .world;
        for (lo..hi) |s| {
            scratch[s] = @as(f64, @floatFromInt(@as(usize, signPattern(grid[s])) % pmod));
        }
        if (@abs(pearson(feat, scratch, lo, hi)) >= EQUIV) return .world;
    }
    return .novel;
...
pub fn main() !void {
    ...
    var prng = std.Random.DefaultPrng.init(0xA7C0DE20260629);
    ...
    try out.print("=== RESEARCH Q7: E11 scale — 100 LLM proposals, engine sole judge ===\n\n", .{});
    try out.print("Proposals file: {s}\n", .{json_path});
    try out.print("Proposals: {d}   Cert threshold: held-out test ≥ {d:.2}\n", .{ proposals.len, CERT });
    try out.print("Targets: parity | hidden_pair (2,5) | sum_mod (%%7) | oriented (v1>v0) | monomial_sign φ{{2,5}}\n", .{});
    try out.print("Split: train {d} / val {d} / test {d}\n", .{ NTR, NVA - NTR, NSAMP - NVA });
    try out.print("Pass bar: ≥2 novel (non Walsh/spectral/monomial/world) on ≥2 distinct families\n\n", .{});

    var results = std.ArrayList(Result).init(alloc);
    defer results.deinit();

    try out.print("──────── certification matrix (certified rows only) ────────\n", .{});
    try out.print("proposal                  | target         | val   | test  | family\n", .{});
    try out.print("--------------------------+----------------+-------+-------+--------\n", .{});
    ...
    for (proposals) |prop| {
        for (0..NT) |ti| {
            ...
            if (certified) {
                try out.print("{s:<25} | {s:<14} | {d:.3} | {d:.3} | {s}\n", .{
                    prop.name,
                    target_name[ti],
                    val,
                    tst,
                    family_name[@intFromEnum(family)],
                });
            }
        }
    }
    ...
    var n_cert: usize = 0;
    var n_novel: usize = 0;
    var cert_by_target: [NT]usize = .{0} ** NT;
    var novel_by_target: [NT]usize = .{0} ** NT;
    var families_with_novel: usize = 0;

    for (results.items) |r| {
        if (!r.certified) continue;
        n_cert += 1;
        const ti = @intFromEnum(r.target);
        cert_by_target[ti] += 1;
        if (r.family == .novel) {
            n_novel += 1;
            novel_by_target[ti] += 1;
        }
    }
    for (0..NT) |ti| {
        if (novel_by_target[ti] > 0) families_with_novel += 1;
    }

    const pass = n_novel >= 2 and families_with_novel >= 2;

    try out.print("\n──────── per-family breakdown ────────\n", .{});
    for (0..NT) |ti| {
        try out.print("  {s:<14}: certified={d}  novel={d}\n", .{ target_name[ti], cert_by_target[ti], novel_by_target[ti] });
    }

    try out.print("\n════════════════════ VERDICT ════════════════════\n", .{});
    try out.print("Proposals submitted : {d}\n", .{proposals.len});
    try out.print("Accepted (certified pairs): {d}\n", .{n_cert});
    try out.print("Novel (non-equivalent)    : {d}\n", .{n_novel});
    try out.print("Families with novel       : {d}\n", .{families_with_novel});
    ...
    if (pass) {
        try out.print("\nPASS — ≥2 non-equivalent promoted features on ≥2 distinct families.\n", .{});
    } else {
        try out.print("\nFAIL — pass bar not met (need ≥2 novel on ≥2 families).\n", .{});
    }

    try out.print("\nSee: docs/research/open_invention_rq7.md, open_invention_e11.md, unified_invention.zig\n", .{});
    try out.print("RQ7_RESULT pass={} accepted={d} novel={d} doc=docs/research/open_invention_rq7.md\n", .{ pass, n_cert, n_novel });
}