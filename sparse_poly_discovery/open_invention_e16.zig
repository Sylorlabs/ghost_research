...
//!
//! Extends E3 (no spectral/Walsh) with a 20-target mod battery:
//!   • mod(sum, 3) anchor
//!   • mod(popcount(sign_bitmap), 4) anchor
//!   • composed mod(f(grid), k) for k ∈ {3, 5, 7} and f ∈ {count, inv, max, sum+count, …}
//!
//! Reuses E3 program synthesis: depth≤4 over {+,×,sin,cos,abs,mod} on scalar leaves
//! {count, sum, pop, inv, max}. No Walsh promotions (0 by construction).
//!
//! PASS bar: ≥12/20 certified via synthesis only (held-out ≥0.90); 0 Walsh promotions.
//!
//! Run: zig build open-invention-e16 --release=fast
...
const PASS_SYNTH: usize = 12;
const RNG_SEED: u64 = 0xE16A801CEF00D;
const NT: usize = 20;
...
const Atom = struct {
    ...
    fn isSynth(self: Atom) bool {
        return self.kind == .synth_prog;
    }
};
...
const TARGETS = [_]Target{
    // mod(sum, k) family — anchor T01 mod(sum,3)
    .{ .name = "T01 sum%3==0", .kind = .sum_mod, .modulus = 3 },
    .{ .name = "T02 sum%5==0", .kind = .sum_mod, .modulus = 5 },
    .{ .name = "T03 sum%7==0", .kind = .sum_mod, .modulus = 7 },
    // mod(popcount, k) family — anchor T04 mod(popcount,4)
    .{ .name = "T04 pop(sign)%4==0", .kind = .pop_mod, .modulus = 4 },
    .{ .name = "T05 pop(sign)%3==0", .kind = .pop_mod, .modulus = 3 },
    .{ .name = "T06 pop(sign)%5==0", .kind = .pop_mod, .modulus = 5 },
    .{ .name = "T07 pop(sign)%7==0", .kind = .pop_mod, .modulus = 7 },
    // composed mod(f(grid), k), k ∈ {3,5,7}
    .{ .name = "T08 count%3==0", .kind = .count_mod, .modulus = 3 },
    .{ .name = "T09 count%5==0", .kind = .count_mod, .modulus = 5 },
    .{ .name = "T10 count%7==0", .kind = .count_mod, .modulus = 7 },
    .{ .name = "T11 inv%3==0", .kind = .inv_mod, .modulus = 3 },
    .{ .name = "T12 inv%5==0", .kind = .inv_mod, .modulus = 5 },
    .{ .name = "T13 inv%7==0", .kind = .inv_mod, .modulus = 7 },
    .{ .name = "T14 max%3==0", .kind = .max_mod, .modulus = 3 },
    .{ .name = "T15 max%5==0", .kind = .max_mod, .modulus = 5 },
    .{ .name = "T16 max%7==0", .kind = .max_mod, .modulus = 7 },
    .{ .name = "T17 (sum+count)%3==0", .kind = .affine_mod, .modulus = 3, .affine_a = 1, .affine_b = 1 },
    .{ .name = "T18 (sum+count)%5==0", .kind = .affine_mod, .modulus = 5, .affine_a = 1, .affine_b = 1 },
    .{ .name = "T19 (sum+count)%7==0", .kind = .affine_mod, .modulus = 7, .affine_a = 1, .affine_b = 1 },
    .{ .name = "T20 (sum+inv)%7==0", .kind = .mixed_affine_mod, .modulus = 7, .affine_a = 1, .affine_b = 0, .affine_c = 1 },
};
...
fn signPopcount(g: [NCELL]u8) usize {
    return popcount(signPattern(g));
}
...
fn targetLabel(g: [NCELL]u8, t: Target) f64 {
    return switch (t.kind) {
        ...
        .pop_mod => if (signPopcount(g) % t.modulus == 0) 1.0 else 0.0,
        .count_mod => if (@as(usize, @intFromFloat(countGE(g))) % t.modulus == 0) 1.0 else 0.0,
        .inv_mod => if (inversionCount(g) % t.modulus == 0) 1.0 else 0.0,
        .max_mod => if (@as(usize, maxCell(g)) % t.modulus == 0) 1.0 else 0.0,
        .affine_mod => blk: {
            const c = @as(usize, @intFromFloat(countGE(g)));
            const s = gridSum(g);
            break :blk if ((t.affine_a * s + t.affine_b * c) % t.modulus == 0) 1.0 else 0.0;
        },
        .mixed_affine_mod => blk: {
            const s = gridSum(g);
            const inv = inversionCount(g);
            break :blk if ((t.affine_a * s + t.affine_b * @as(usize, @intFromFloat(countGE(g))) + t.affine_c * inv) % t.modulus == 0) 1.0 else 0.0;
        },
    };
}

// ── E3 program synthesis (depth ≤ 4, leaves = count/sum/pop/inv/max) ─────────

const Leaf = enum { count, sum, pop, inv, max, sum_count, sum_inv };
...
const ScalarCtx = struct {
    ...
    fn leafVal(self: ScalarCtx, leaf: Leaf) f64 {
        return switch (leaf) {
            ...
            .sum_inv => self.sum_inv,
        };
    }

    fn fromGrid(g: [NCELL]u8) ScalarCtx {
        const c = countGE(g);
        const s: f64 = @floatFromInt(gridSum(g));
        const inv: f64 = @floatFromInt(inversionCount(g));
        return .{
            .count = c,
            .sum = s,
            .pop = @floatFromInt(signPopcount(g)),
            .inv = inv,
            .max = @floatFromInt(maxCell(g)),
            .sum_count = s + c,
            .sum_inv = s + inv,
        };
    }
...
const ProgBank = struct {
    ...
    fn eval(self: ProgBank, id: u16, ctx: ScalarCtx) f64 {
        return switch (self.nodes[id]) {
            ...
            .cos => |ch| @cos(self.eval(ch, ctx)),
            .abs => |ch| @abs(self.eval(ch, ctx)),
...
fn buildProgBank(alloc: std.mem.Allocator) !ProgBank {
    ...
    const leaf_tags = [_]struct { leaf: Leaf, tag: []const u8 }{
        .{ .leaf = .count, .tag = "count" },
        .{ .leaf = .sum, .tag = "sum" },
        .{ .leaf = .pop, .tag = "pop" },
        .{ .leaf = .inv, .tag = "inv" },
        .{ .leaf = .max, .tag = "max" },
        .{ .leaf = .sum_count, .tag = "sum+count" },
        .{ .leaf = .sum_inv, .tag = "sum+inv" },
    };
    for (leaf_tags) |lt| {
...
fn evalAtom(g: [NCELL]u8, atom: Atom, bank: ProgBank) f64 {
    ...
    return switch (atom.kind) {
        ...
        .count_feat => ctx.count - @as(f64, @floatFromInt(NCELL)) / 2.0,
        .pop_feat => ctx.pop - @as(f64, @floatFromInt(NCELL)) / 2.0,
        .inv_count_feat => ctx.inv - @as(f64, @floatFromInt(NCELL * (NCELL - 1) / 2)) / 2.0,
        .max_all_feat => ctx.max - MID,
        .min_pair => blk: {
            const i: usize = @intCast(atom.data / 8);
            const j: usize = @intCast(atom.data % 8);
...
fn libraryHasSynth(atoms: []const Atom) bool {
    for (atoms) |a| {
        if (a.isSynth()) return true;
    }
    return false;
}
...
fn discoverBest(
    ...
    tryCandidate(.{ .kind = .pop_feat, .data = 0 }, current, grid, X, Yt, bank, w, &best_val, &best);
...
const ForgeResult = struct {
    ...
    test_acc: [NT]f64,
    escape_atom: [NT]?Atom,
    pre_synth_solved: [NT]bool,
    natoms: usize,
    saturated: bool,
    synth_promotions: usize,
    walsh_promotions: usize,
};
...
fn runForge(
    ...
    var pre_synth_solved = [_]bool{false} ** NT;
    ...
    var saturated = false;
    var synth_on = false;
    var synth_promotions: usize = 0;
    const walsh_promotions: usize = 0;
    ...
    while (round <= 14) : (round += 1) {
        ...
        for (0..NT) |t| {
            ...
            if (escape and irreducible and natoms < MAXATOMS) {
                ...
                if (disc.atom.isSynth()) synth_promotions += 1;
                promoted = true;
        ...
        if (!promoted) {
            if (!synth_on) {
                for (0..NT) |t| pre_synth_solved[t] = solved[t];
                synth_on = true;
                try out.print("  r{d}: monomial+stats saturated → enable E3 synthesis (depth≤4, {{+,×,sin,cos,abs,mod}})\n", .{round});
                continue;
            }
    ...
    return .{
        ...
        .pre_synth_solved = pre_synth_solved,
        .natoms = natoms,
        .saturated = saturated,
        .synth_promotions = synth_promotions,
        .walsh_promotions = walsh_promotions,
    };
}

fn isSynthCertified(result: ForgeResult, t: usize, has_synth_lib: bool) bool {
    if (!result.solved[t]) return false;
    if (result.escape_atom[t]) |a| return a.isSynth();
    return !result.pre_synth_solved[t] and has_synth_lib;
}
...
pub fn main() !void {
    ...
    try out.print("=== EXPERIMENT E16: mod synthesis beyond parity (E3++) ===\n\n", .{});
    try out.print("RNG seed = 0x{X:0>16} (pinned)\n", .{RNG_SEED});
    try out.print("Anchors: mod(sum,3), mod(popcount(sign),4), composed mod(f,k) k∈{{3,5,7}}.\n", .{});
    try out.print("Menu: monomials, thresh, sums, min/max, rank stats, pop/inv/max scalars.\n", .{});
    try out.print("Synthesis (E3 reuse): depth≤4 {{+,×,sin,cos,abs,mod}} on {{count,sum,pop,inv,max,sum+count,sum+inv}}.\n", .{});
    ...
    try out.print("PASS bar: ≥{d}/{d} certified via synthesis only; 0 Walsh promotions.\n", .{ PASS_SYNTH, NT });
    try out.print("Train/val/test {d}/{d}/{d}\n\n", .{ NTR, NVA - NTR, NSAMP - NVA });

    try out.print("── mod target battery ──\n", .{});
    for (TARGETS, 0..) |t, i| {
        var pos: usize = 0;
        for (0..NSAMP) |s| {
            if (Y[i][s] > 0.5) pos += 1;
    ...
    const has_synth_lib = result.synth_promotions > 0;

    var synth_certified: usize = 0;
    var total_certified: usize = 0;
    var mono_only: usize = 0;
    ...
    for (0..NT) |t| {
        ...
        const feat_str: []const u8 = if (result.escape_atom[t]) |a|
            ...
            "—";
        const synth_ok = isSynthCertified(result, t, has_synth_lib);
        if (result.solved[t]) {
            total_certified += 1;
            if (synth_ok) synth_certified += 1 else mono_only += 1;
        }
        try out.print("  {s}: test={d:.3} cert={} synth_cert={} escape={s}\n", .{
            TARGETS[t].name,
            result.test_acc[t],
            result.solved[t],
            synth_ok,
    ...
    const pass = synth_certified >= PASS_SYNTH and result.walsh_promotions == 0;

    try out.print("\n════════════════════ VERDICT (E16) ════════════════════\n", .{});
    try out.print("seed 0x{X:0>16}\n", .{RNG_SEED});
    try out.print("total certified: {d}/{d}\n", .{ total_certified, NT });
    try out.print("synthesis-certified: {d}/{d} (need ≥{d})\n", .{ synth_certified, NT, PASS_SYNTH });
    try out.print("monomial-only certified: {d}\n", .{mono_only});
    try out.print("synth promotions: {d}; Walsh promotions: {d}\n", .{ result.synth_promotions, result.walsh_promotions });
    try out.print("atoms promoted: {d}; saturated={}\n\n", .{ result.natoms, result.saturated });

    if (pass) {
        try out.print("PASS — {d}/{d} mod targets certified via synthesis without Walsh.\n", .{ synth_certified, NT });
        try out.print("E3++ escape generalizes mod(sum), mod(popcount), and composed mod(f,k).\n", .{});
    } else {
        try out.print("FAIL — synthesis-certified {d}/{d} (need ≥{d}); Walsh promotions={d}.\n", .{
            synth_certified, NT, PASS_SYNTH, result.walsh_promotions,
        });
        try out.print("Some mod families may need richer leaves or outside generators.\n", .{});
    }

    try out.print("\nE16_RESULT pass={} solve_total={d} solve_synth={d} walsh_promo={d} doc=docs/research/open_invention_e16.md\n", .{
        pass, total_certified, synth_certified, result.walsh_promotions,
    });
    try out.print("See: open_invention_e16.md, open_invention_e3.md, open_invention_rq3.md.\n", .{});
}