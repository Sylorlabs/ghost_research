...
const domain = @import("domain_128_arx");

pub fn main() !void {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();

    var prng = std.Random.DefaultPrng.init(0x1337_BEEF_CAFE_0128);
    var seed = prng.random().int(u64);

    const out = std.io.getStdOut().writer();
    try out.print("=== Structurally Bijective 128-bit ARX Synthesis ===\n", .{});
    try out.print("Domain: {s}\n", .{domain.DOMAIN_NAME});
    try out.print("Goal: Synthesize a high-quality bijective 128-bit PRNG state transition.\n\n", .{});

    const Iters = 20000;
    var best = domain.randomProgram(&seed, 24);
    ...
    var i: usize = 1;
    while (i <= Iters) : (i += 1) {
        const candidate = domain.mutate(best, &seed, 24);
    ...
    var csv_file = try std.fs.cwd().createFile("../results/arx_128_champion.csv", .{});
    defer csv_file.close();
    try domain.programToCsv(best, csv_file.writer());
    try out.print("Saved to ../results/arx_128_champion.csv\n", .{});
}