...
pub fn main() !void {
    ...
    // Generate sequence from an initial state
    var state = domain.State{ .r0 = 0xCAFEBABE12345678, .r1 = 0xDEADBEEF9ABCDEF0 };
    var written: u64 = 0;
    const buf_len = 8192;
    ...
    while (written < bytes) {
        ...
        while (i < buf_len) : (i += 1) {
            ...
            buf[i] = state.r0 ^ state.r1; // Collapse 128-bit state to 64-bit output
        }
