...
pub fn main() !void {
    ...
    try o.print("=== LM-BPC5 — DATA SCALING: is the gap to gpt2 a DATA problem? Sweep warm-start size. dewrap={} ===\n\n", .{dewrap});

    const cap: usize = if (trainMB > 0) trainMB * 1_000_000 else 40_000_000;
    var tr = std.ArrayList(u8).init(a);
    // load the 3 base books + every novel in corpus/biglit/ (all non-Austen 19th-c English; held-out is Austen)
    ...
    {
        const bl = try std.fs.path.join(a, &.{ dir, "biglit" });
        var d = std.fs.openDirAbsolute(bl, .{ .iterate = true }) catch unreachable;
        defer d.close();
        var it = d.iterate();
        while (try it.next()) |entry| {
            if (entry.kind != .file or !std.mem.endsWith(u8, entry.name, ".txt")) continue;
            if (tr.items.len >= cap) break;
            const p = try std.fs.path.join(a, &.{ bl, entry.name });
            const f = std.fs.openFileAbsolute(p, .{}) catch continue;
            defer f.close();
            try tr.appendSlice(try f.readToEndAlloc(a, 1 << 30));
        }
