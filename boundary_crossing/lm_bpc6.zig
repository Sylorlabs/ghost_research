...
pub fn main() !void {
    ...
    try o.print("=== LM-BPC6 — EQUAL DATA: count model on the SAME 8MB file as the from-scratch GPT (byte-identical) ===\n\n", .{});

    const tf = try std.fs.openFileAbsolute("/home/micah/Desktop/Sylorlabs/ghost_research/corpus/train_dw_8mb.txt", .{});
    const TRB = try tf.readToEndAlloc(a, 1 << 30);
    tf.close();
    const hf = try std.fs.openFileAbsolute("/home/micah/Desktop/Sylorlabs/ghost_research/corpus/heldout_eval_dw.txt", .{});
    ...
    try o.print("  OUR COUNT MODEL on the SAME 8MB:  FROZEN BPB = {d:.4}   PREQUENTIAL BPB = {d:.4}\n", .{ bf, bp });
    try o.print("  vs a from-scratch GPT trained ONLY on this same 8MB (see nanogpt_bpb.py) — EQUAL DATA, equal playing field.\n", .{});
    try o.print("  (reference: a STOLEN gpt2 pretrained on billions of tokens = 1.0499, but that's NOT equal data.)\n", .{});
}
