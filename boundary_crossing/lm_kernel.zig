...
// ── MATHEMATICAL ENCODER: Random Fourier Features (Rahimi–Recht). z(x)=cos(Wr·x+br) approximates an RBF kernel's
//    infinite-dim feature map — nonlinear "depth" with NO backprop-through-depth; only the linear readout Θ is trained
//    (convex). Order-aware input = CONCAT of the last CTX rune embeddings. Parametric → FIXED memory → scales to any data.
const ED: usize = 24; // rune embedding dim
const CTX: usize = 3; // context runes (concatenated → order-aware)
const CIN: usize = CTX * ED;
const M: usize = 512; // random features
const KSCALE: f32 = 0.8; // RBF bandwidth (Wr ~ N(0, KSCALE²))
var emb: []f32 = undefined; // [V*ED] PPMI rune embeddings
var Wr: []f32 = undefined; // [M*CIN] random projection (fixed)
var br: []f32 = undefined; // [M] random phase (fixed)
var Th: []f32 = undefined; // [V*M] learned readout
var Vg: usize = 0;
var zbuf: []f32 = undefined;
var rngK: u64 = 0xC0FFEE123;
fn frngK() f32 {
    rngK ^= rngK << 13;
    rngK ^= rngK >> 7;
    rngK ^= rngK << 17;
    return (@as(f32, @floatFromInt(rngK % 1_000_000)) / 1_000_000.0);
}
fn gaussK() f32 { // approx N(0,1) via sum of 6 uniforms
    var s: f32 = 0;
    for (0..6) |_| s += frngK();
    return (s - 3.0) * 0.7071;
}
fn rffFeat(R: []const u32, p: usize, z: []f32) void {
    var x = [_]f32{0} ** CIN;
    for (0..CTX) |d| {
        const u: usize = R[p - 1 - d];
        for (0..ED) |k| x[d * ED + k] = emb[u * ED + k];
    }
    for (0..M) |j| {
        var s: f32 = br[j];
        const base = j * CIN;
        for (0..CIN) |k| s += Wr[base + k] * x[k];
        z[j] = @cos(s);
    }
}
fn rffProb(R: []const u32, p: usize, r: u32) f64 { // softmax over V of Θ·z (probabilistic readout)
    rffFeat(R, p, zbuf);
    var smax: f64 = -1e30;
    var sr: f64 = 0;
    for (0..Vg) |c| {
        var s: f32 = 0;
        const base = c * M;
        for (0..M) |j| s += Th[base + j] * zbuf[j];
        if (c == r) sr = s;
        if (@as(f64, s) > smax) smax = s;
    }
    var sum: f64 = 0;
    for (0..Vg) |c| {
        var s: f32 = 0;
...
pub fn main() !void {
    ...
    try o.print("=== LM-KERNEL — random Fourier feature encoder (nonlinear depth, convex readout) + counts. dewrap={} ===\n\n", .{dewrap});
    ...
    // ── build the RFF kernel encoder: PPMI embeddings, fixed random features, train the linear readout Θ on TR ──
    Vg = V;
    ...
    Wr = try std.heap.page_allocator.alloc(f32, M * CIN);
    br = try std.heap.page_allocator.alloc(f32, M);
    Th = try std.heap.page_allocator.alloc(f32, V * M);
    zbuf = try a.alloc(f32, M);
    for (0..M * CIN) |i| Wr[i] = gaussK() * KSCALE;
    for (0..M) |j| br[j] = frngK() * 6.2831853;
    @memset(Th, 0);
    { // train Θ by negative-sampling SGD over TR (RFF fixed; only Θ learns — convex)
        const KLR: f32 = 0.05;
        const KNEG: usize = 5;
        var ep: usize = 0;
        while (ep < 2) : (ep += 1) {
            ...
            while (p < TR.len) : (p += 1) {
                ...
                rffFeat(TR, p, zbuf);
                var sp: f32 = 0;
                for (0..M) |j| sp += Th[trg * M + j] * zbuf[j];
                const gp = 1.0 - 1.0 / (1.0 + @exp(-sp));
                for (0..M) |j| Th[trg * M + j] += KLR * gp * zbuf[j];
                for (0..KNEG) |_| {
                    rngK ^= rngK << 13;
                    rngK ^= rngK >> 7;
                    rngK ^= rngK << 17;
                    const nge: usize = rngK % V;
                    if (nge == trg) continue;
                    var sn: f32 = 0;
                    for (0..M) |j| sn += Th[nge * M + j] * zbuf[j];
                    const gn = 1.0 / (1.0 + @exp(-sn));
                    for (0..M) |j| Th[nge * M + j] -= KLR * gn * zbuf[j];
                }
            }
        }
    }

    // FROZEN pass (no updates on held-out) — count-only vs count+RFF
    var frozenBits: f64 = 0; // count only
    var frozenK: f64 = 0; // count + RFF kernel
    {
        var p: usize = ORD;
        while (p < EV.len) : (p += 1) {
            const cph = evPhOf[p];
            ...
            const pc2 = pFinal(EV, p, EV[p], prevP, prevC);
            frozenBits += -std.math.log2(@max(pc2, 1e-12));
            const pk = 0.70 * pc2 + 0.30 * rffProb(EV, p, EV[p]);
            frozenK += -std.math.log2(@max(pk, 1e-12));
        }
    }
    ...
    var scored: usize = 0;
    {
        ...
        while (p < EV.len) : (p += 1) {
            ...
            const prevC: u32 = if (cco > 0) evCO[cco - 1] else 0;
            const pc2 = pFinal(EV, p, EV[p], prevP, prevC);
            preqBits += -std.math.log2(@max(pc2, 1e-12));
            preqK += -std.math.log2(@max(0.70 * pc2 + 0.30 * rffProb(EV, p, EV[p]), 1e-12));
            scored += 1;
    ...
    const bfk = frozenK / @as(f64, @floatFromInt(nbytes));
    const bpk = preqK / @as(f64, @floatFromInt(nbytes));
    try o.print("  count-only       FROZEN {d:.4}   PREQUENTIAL {d:.4}\n", .{ bf, bp });
    try o.print("  + RFF KERNEL     FROZEN {d:.4}   PREQUENTIAL {d:.4}   (M={d} random features, ED={d}, CTX={d})\n", .{ bfk, bpk, M, ED, CTX });
    try o.print("  RFF kernel Δ:    frozen {s}{d:.4}   prequential {s}{d:.4}   (negative = the kernel encoder HELPED)\n\n", .{ if (bfk - bf >= 0) "+" else "", bfk - bf, if (bpk - bp >= 0) "+" else "", bpk - bp });
    try o.print("  gpt2-124M ({s}) = {d:.4}; best-ours Δ vs gpt2: frozen {s}{d:.4}\n", .{ if (dewrap) "FAIR de-wrapped" else "WRAPPED-confounded", gpt2, if (@min(bf, bfk) - gpt2 >= 0) "+" else "", @min(bf, bfk) - gpt2 });
}
