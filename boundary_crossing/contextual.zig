...
pub fn main() !void {
    ...
    try o.print("forged {d} runes, strong embeddings over {d:.1}MB.\n", .{ nv - 256, @as(f64, @floatFromInt(buf.len)) / 1e6 });
    try o.print("Does CONTEXTUALIZING the occurrence (non-parametric self-attention) lift WSD over plain averaging? (50%=chance)\n\n", .{});
    ...
    const cen = try a.alloc(f32, nr); // reused centroid buffer for the attention
    var sa: f64 = 0;
    var ss: f64 = 0;
    var sw: f64 = 0;
    var npairs: usize = 0;
    try o.print("  {s:<16} {s:>5}  {s:>7}  {s:>10}  {s:>8}\n", .{ "pseudoword", "sim", "avg", "self-attn", "wide-win" });
    for (ps) |pr| {
        const ra = findRune(pr[0]);
        const rb = findRune(pr[1]);
        if (ra < 0 or rb < 0) continue;
        const sim = cosv(@intCast(ra), @intCast(rb));
        var pur = [_]f32{ 0, 0, 0 };
        for (0..3) |method| {
            const win: usize = if (method == 2) 7 else W;
            var M: usize = 0;
            var i: usize = 0;
            while (i < cs.len) : (i += 1) {
                if (M >= MAXOCC) break;
                ...
                if (method == 1) {
                    // non-parametric SELF-ATTENTION: weight each context rune by its coherence with the context
                    // centroid (attend to the relevant core, down-weight outliers). No learned Q/K/V — pure similarity.
                    var nb: [16]usize = undefined;
                    var nn: usize = 0;
                    var d: usize = 1;
                    while (d <= W) : (d += 1) {
                        if (i >= d and cs[i - d] >= 0 and nn < 16) {
                            nb[nn] = @intCast(cs[i - d]);
                            nn += 1;
                        }
                        if (i + d < cs.len and cs[i + d] >= 0 and nn < 16) {
                            nb[nn] = @intCast(cs[i + d]);
                            nn += 1;
                        }
                    }
                    if (nn == 0) continue;
                    @memset(cen, 0);
                    for (0..nn) |g| for (0..nr) |k| {
                        cen[k] += mat[nb[g] * nr + k];
                    };
                    var cnrm: f32 = 0;
                    for (0..nr) |k| cnrm += cen[k] * cen[k];
                    if (cnrm > 0) {
                        const cinv = 1.0 / @sqrt(cnrm);
                        for (0..nr) |k| cen[k] *= cinv;
                    }
                    var Z: f32 = 0;
                    var wts: [16]f32 = undefined;
                    for (0..nn) |g| {
                        var s: f32 = 0;
                        for (0..nr) |k| s += mat[nb[g] * nr + k] * cen[k];
                        wts[g] = @exp(5.0 * s);
                        Z += wts[g];
                    }
                    if (Z <= 0) continue;
                    for (0..nn) |g| {
                        const wn = wts[g] / Z;
                        for (0..nr) |k| ctx[base + k] += wn * mat[nb[g] * nr + k];
                    }
                } else {
                    var any = false;
                    var d: usize = 1;
                    while (d <= win) : (d += 1) {
                        if (i >= d and cs[i - d] >= 0) {
                            const nb2: usize = @intCast(cs[i - d]);
                            for (0..nr) |k| ctx[base + k] += mat[nb2 * nr + k];
                            any = true;
                        }
            ...
            if (M >= 20) pur[method] = try splitPurity(a, ctx, lab, M);
        }
        var nm: [40]u8 = undefined;
        const label = std.fmt.bufPrint(&nm, "{s}+{s}", .{ pr[0], pr[1] }) catch pr[0];
        try o.print("  {s:<16} {d:>5.2}  {d:>6.0}%  {d:>9.0}%  {d:>7.0}%\n", .{ label, sim, 100 * pur[0], 100 * pur[1], 100 * pur[2] });
        sa += pur[0];
        ss += pur[1];
        sw += pur[2];
        npairs += 1;
    }
    const dn = @as(f64, @floatFromInt(@max(1, npairs)));
    try o.print("\n  MEAN:  avg {d:.0}%   self-attn {d:.0}%   wide-win {d:.0}%\n", .{ 100 * sa / dn, 100 * ss / dn, 100 * sw / dn });

    try o.print("\n════════════════════ VERDICT ════════════════════\n", .{});
    try o.print("The capability gap to a transformer is CONTEXT-DEPENDENCE, and attention is just similarity-weighted\n", .{});
    try o.print("averaging — so it's reachable count-based, no learned weights. This tests it: 'self-attn' = each context\n", .{});
    try o.print("rune weighted by coherence with the context core (non-parametric attention). Compare the means above to\n", .{});
    try o.print("plain averaging. If it moves the number, the contextual lever is real and cheap; if not on THIS yardstick,\n", .{});
    try o.print("the honest read is that pseudoword-merge mutes the per-occurrence signal attention needs (the two words\n", .{});
    try o.print("share the same slot), so the place attention pays off is FULL contextual encoding + RETRIEVAL over the\n", .{});
    try o.print("datastore (kNN-LM / RETRO) — where non-parametric models genuinely MATCH or BEAT transformers on perplexity\n", .{});
    try o.print("and online-learning, because the datastore IS the model (all data at inference, updatable, no forgetting).\n", .{});
    try o.print("That is the real architectural bet: contextual non-parametric attention + retrieval, with at most a CHEAP\n", .{});
    try o.print("LEARNED METRIC — not GPU pretraining. Measured next on a real WSD/LM benchmark, head-to-head. No LLM.\n", .{});
}
