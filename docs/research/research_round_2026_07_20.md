...
**Status:** LIVE — AS1/AS2 are gate-ready adapters; AS3 and AS4 are bounded local-artifact positives; AS5 is a valid negative; AS6 independent reduction audit is running.
...
| AS6 | Luna medium | Real-workbench reduction audit | Can an independent critic reduce any apparent local/web discovery to cached answer, source leak, fixed parser rule, broad search, post-hoc claim, or capture artifact? | **RUNNING** | Only claims surviving source/web provenance, replay, cost, alternate explanation, and scope attacks remain. | `docs/research/real_workbench_reduction_audit_round_as.md`, `results/real_workbench_reduction_audit_round_as.csv`, `sparse_poly_discovery/real_workbench_reduction_audit_round_as.zig` |
