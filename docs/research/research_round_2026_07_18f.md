...
**Status:** PARTIALLY LANDED — AP1/AP2 are gate-ready for measured direct syscall/PID/resource restrictions; AP3 is inconclusive because vDSO clocks bypass syscall filtering. AP4 is ready to attack the combined launcher and measure that residual live.
...
| AP4 | Terra medium | Post-hardening hostile audit | Does the AP launcher survive AO4's full hostile runtime suite, including clock and fork, with evaluator integrity intact? | ready (AP1/AP2/AP3) | All executed attack routes deny/contain; transcript holds; must explicitly test direct and vDSO time plus fork. Residual TCB is measured and named. | `docs/research/post_hardening_audit_round_ap.md`, `results/post_hardening_audit_round_ap.csv`, `sparse_poly_discovery/post_hardening_audit_round_ap.zig` |
