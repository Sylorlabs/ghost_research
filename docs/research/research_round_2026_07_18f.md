...
**Status:** COMPLETE — AP4 is gate-ready within the declared local Linux/process threat model; AP5 and AP6 reproduce AM2/AM4/AM5 through a separate candidate process at original budgets. vDSO/physical timing remains an explicit residual.
...
| AP6 | Terra medium | Isolated AM4/AM5 replay | Do causal allocation and portfolio transfer survive an attacked, hardened boundary at their original equal budgets? | **FOUNDATION POSITIVE (bounded)** | Separate candidate preserves AM4 **3,600** at 1,920 contacts over broad 1,380 and all controls; preserves AM5 **5,760** at 4,320 contacts over shuffled 1,680, single 1,620, and same-evidence wrong route 0. | `docs/research/isolated_allocation_portfolio_round_ap.md`, `results/isolated_allocation_portfolio_round_ap.csv`, `sparse_poly_discovery/isolated_allocation_portfolio_round_ap.zig` |
...
**Round AP closes the evaluator-isolation repair at a bounded local foundation tier.** AP4 independently attacks the combined Bubblewrap/seccomp/rlimit/transcript profile and finds no executed evaluator-data or corruption route. AP5 then reproduces AM2 topology through a separate candidate process at its original budget. AP6 reproduces both AM4 allocation (3,600 vs strongest broad coverage 1,380 at 1,920 contacts) and AM5 portfolio (5,760 vs strongest shuffled 1,680 at 4,320 contacts; same-evidence wrong route 0) through the same process separation. Fresh coordinator builds, smoke tests, and two independent full ledgers byte-compare.

This is **not** a claim of absolute security, open-ended invention, scientific novelty, or general intelligence. vDSO/physical timing, deny-list/unlisted syscalls, dynamic loader/bootstrap, Bubblewrap/seccomp/kernel/compiler/hardware, and synthetic-world scope remain explicit residual/trusted surfaces. The next capability work may use this boundary to run no-answer-key discovery pilots, but must continue to evaluate claims through precommitment, intervention, replication, alternatives, and independent replay—not human answer matching.
