...
**Status:** LIVE — M1–M3 complete and coordinator-verified; M4 proposal grammar is running, with M5/M6 gated behind it.
...
| M4 | Luna medium | Trace-derived proposal grammar | Can split-region traces generate a small nonredundant candidate language without a supplied missing-family name? | **RUNNING** | Proposal beats frozen existing menu on fresh test cells in a declared split region. | `docs/research/proposal_grammar_round_m.md`, `results/proposal_grammar_round_m.csv`, `sparse_poly_discovery/proposal_grammar_round_m.zig` |
| M5 | Terra medium | Enforced closed-loop expedition | Does map → split → propose → query → fresh test beat fixed coverage at equal enforced cost? | blocked on M1/M2/M3/M4 | Strict fresh-test win over fixed and blind controls with persistent budget and complete ledger. | `docs/research/closed_loop_round_m.md`, `results/closed_loop_round_m.csv`, `sparse_poly_discovery/closed_loop_round_m.zig` |
| M6 | Terra medium | Independent audit | Do evaluator, split, transfer, proposal, and closed-loop claims survive leakage/accounting/baseline attacks? | blocked on M1/M2/M3/M4/M5 | Claim-by-claim replay; no autonomy claim without pass. | `docs/research/round_m_audit.md`, `results/round_m_audit.csv`, `sparse_poly_discovery/round_m_audit.zig` |
