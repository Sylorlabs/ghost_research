...
// Synthesized from recursive_conceptless_inventor_v2.zig (which was itself
...
//   v2 adds pair-flip frontier search and reaches 296 in the recorded
//   aggressive sweep. v3 adds bounded exhaustive triple-flip frontier search
//   after pair-flip saturation. The cost is deliberately exposed as
//   --triple-steps because one step is ~22M triples * RefCount.
...
// Parent metrics (recursive_conceptless_inventor_v2's best from the recorded
// aggressive sweep):
...
const ChildSalt: u64 = 0x7A91_3F11_1E55_0003;
...
fn candidateDistsTriple(field: Field, refs: []const Field, dists: Distances, b1: usize, b2: usize, b3: usize) Distances {
    var next = dists;
    const old1 = bitAt(field, b1);
    const old2 = bitAt(field, b2);
    const old3 = bitAt(field, b3);
    for (refs, 0..) |ref, idx| {
        if (old1 == bitAt(ref, b1)) next[idx] += 1 else next[idx] -= 1;
        if (old2 == bitAt(ref, b2)) next[idx] += 1 else next[idx] -= 1;
        if (old3 == bitAt(ref, b3)) next[idx] += 1 else next[idx] -= 1;
    }
    return next;
}
...
fn tripleFrontierStep(field: *Field, refs: []const Field, dists: *Distances, frontier: u32) bool {
    const cost_now = deficitCost(dists[0..refs.len], frontier);
    if (cost_now == 0) return false;
    ...
    while (b1 < Dim) : (b1 += 1) {
        ...
        while (b2 < Dim) : (b2 += 1) {
            var b3: usize = b2 + 1;
            while (b3 < Dim) : (b3 += 1) {
                const cand = candidateDistsTriple(field.*, refs, dists.*, b1, b2, b3);
                const cand_cost = deficitCost(cand[0..refs.len], frontier);
                const cand_stats = distStats(cand[0..refs.len]);
                if (cand_cost < best_cost or (cand_cost == best_cost and cand_stats.score > best_stats.score)) {
                    best_b1 = b1;
    ...
    if (best_b1) |b1f| {
        applyFlip(field, refs, dists, b1f);
        applyFlip(field, refs, dists, best_b2);
        applyFlip(field, refs, dists, best_b3);
        return true;
    }
    return false;
}

fn pushFrontierTriple(field: *Field, refs: []const Field, dists: *Distances, max_triple_steps: usize) u32 {
    var current_min = distStats(dists[0..refs.len]).min;
    var frontier = current_min + 1;
    var step: usize = 0;
    while (step < max_triple_steps) : (step += 1) {
        const moved = tripleFrontierStep(field, refs, dists, frontier);
        if (!moved) break;
        const new_min = distStats(dists[0..refs.len]).min;
        if (new_min >= frontier) {
            frontier += 1;
...
fn invent(seed: u64, refs: []const Field, laws: []const Law, steps: usize, phases: usize, pair_steps: usize, triple_steps: usize, kicks: usize) struct { field: Field, metrics: Metrics } {
    ...
    _ = pushFrontierTriple(&best_field, refs, &best_dists, triple_steps);
    ...
    while (phase < phases) : (phase += 1) {
        ...
        _ = pushFrontierTriple(&field, refs, &dists, triple_steps);
...
pub fn main() !void {
    ...
    try stdout.print("=== RECURSIVE CONCEPTLESS INVENTOR v3 ===\n", .{});
    ...
    try stdout.print("trials={d} steps={d} phases={d} pair_steps={d} triple_steps={d} kicks={d}\n", .{ trials, steps, phases, pair_steps, triple_steps, kicks });
    ...
    while (t < trials) : (t += 1) {
        ...
        const result = invent(trial_seed, refs[0..], laws[0..], steps, phases, pair_steps, triple_steps, kicks);
    ...
    try stdout.print("\nVERDICT: v3_past_parent={d}/{d}; best_min_ref={d}; best_clearance=+{d}; best_violations={d}; best_score={d}; best_hash=0x{X}; csv={s}\n", .{
