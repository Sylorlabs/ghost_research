...
// seeds and averaging the best sort-network quality (composite) found.
...
fn evaluateMetaOnSeeds(meta: tier0.MetaProgram, seeds: []const u64, inner_steps: u32) f64 {
    ...
    for (seeds) |s| {
        ...
        if (std.math.isFinite(q)) { total += q; count += 1; }
    }
...
pub fn main() !void {
    ...
    var out_subdir: []const u8 = "mmm_holdout_hillclimb_sort";
    ...
    try stdout.writeAll("=== SORT NET MMMP HOLDOUT HILLCLIMB (Exp6) ===\n");
    try stdout.print("iters={d} seed=0x{X} mmm_outer={d} tier1_outer={d} tier0_inner={d} constrained={} constrained_meta={} constrained_mm={} wide_meta={} wide_mm={}\n", .{
        iters, root_seed, mmm_outer_iters, tier1_outer_iters, tier0_inner_steps,
        constrained_init, constrained_meta_init, constrained_mm_init, wide_call_meta, wide_call_mm,
    });
    ...
    while (iter <= iters) : (iter += 1) {
        rng = smix(rng);
        var cand = mmm.mutateMetaMetaMeta(current.champion_mmm, &rng);
        ...
        const accepted = r.holdout_mean > current.holdout_mean + 0.01 or
            (r.holdout_mean >= current.holdout_mean - 0.01 and r.anchor_mean > current.anchor_mean + 0.1);
        ...
        try csv.print("{d},{},{},{d:.6},{d:.6},{d:.6},{d:.6}\n", .{
            iter, accepted, best_updated, r.anchor_mean, r.holdout_mean, current.holdout_mean, best.holdout_mean,
        });
        try stdout.print("iter {d}/{d}: anchor={d:.4} holdout={d:.4} current={d:.4} best={d:.4} {s}{s}\n", .{
            iter, iters, r.anchor_mean, r.holdout_mean, current.holdout_mean, best.holdout_mean,
    ...
    try stdout.print("BEST_HOLDOUT = {d:.4} iter={d} anchor={d:.4}\n", .{
        best.holdout_mean, best.iter, best.anchor_mean,
    });
}
