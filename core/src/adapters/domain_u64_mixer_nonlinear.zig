...
pub const DOMAIN_NAME: []const u8 = "u64-mixer-nonlinear-escape";
...
pub fn evaluateQuality(p: Program) Quality {
    ...
    // Challenge 2: Nonlinear Escape
    // Reward structural nonlinearity to force the engine out of the GF(2) affine subspace.
    var non_affine_count: usize = 0;
    var i: usize = 0;
    while (i < p.used) : (i += 1) {
        switch (p.instructions[i].op) {
            .ADD, .AND_NOT, .ADD_ROT => non_affine_count += 1,
            else => {},
        }
    }
    const non_affine_reward = @as(f64, @floatFromInt(non_affine_count)) * 15.0; // Heavy reward

    const composite = -10.0 * av_err - 5.0 * bal_err + 50.0 * per_s - cs_pen - @as(f64, @floatFromInt(p.used)) * 0.5 + non_affine_reward;
