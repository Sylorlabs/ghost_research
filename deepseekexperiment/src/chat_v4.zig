...
pub fn main() !void {
    ...
    // CHAT_V4: tokenize a prompt (file test_prompt.txt), predict next token(s).
    var tokzr = try Tokenizer.init(alloc, "tokenizer.json");
    defer tokzr.deinit();
    const pf = try std.fs.cwd().openFile("test_prompt.txt", .{});
    const ptext = try pf.readToEndAlloc(alloc, 1 << 16);
    pf.close();
    defer alloc.free(ptext);
    const trimmed = std.mem.trimRight(u8, ptext, "\n ");
    var pids = std.ArrayList(u32).init(alloc);
    defer pids.deinit();
    try tokzr.encode(trimmed, &pids);
    const prompt_len = pids.items.len;
    std.debug.assert(prompt_len > 0 and prompt_len < ntok);
    const tokens = try alloc.alloc(u32, ntok + 1);
    defer alloc.free(tokens);
    for (0..ntok + 1) |i| tokens[i] = if (i < prompt_len) pids.items[i] else pids.items[prompt_len - 1];

    try stdout.print("=== CHAT_V4: DeepSeek V4 Pro (XOR-quant) next-token prediction ===\n", .{});
    ...
    // ===== head: next-token prediction at the last prompt position =====
    ...
    // QUANT stream (s=1) — the XOR-quantized model — at the last prompt token
    const t = prompt_len - 1;
    var xf: [DIM]f32 = undefined;
    {
        const hi = h[(t * 2 + 1) * HCDIM .. (t * 2 + 1 + 1) * HCDIM];
        var ssum: f64 = 0;
        ...
        @memset(&xf, 0);
        for (0..HC) |j| {
            for (0..DIM) |i| xf[i] += pre[j] * hi[j * DIM + i];
        }
        rmsNormApply(&xf, final_norm);
    }
    var row: [DIM]f32 = undefined;
    for (0..VOCAB) |r| {
        bf16Row(head_w, r, &row);
        logits[r] = @floatCast(dotF64(&row, &xf));
    }
    // top-5 next tokens
    try stdout.print("\n=== XOR-V4 predicts next token (top-5) ===\n", .{});
    var picked = [_]bool{false} ** 8;
    _ = &picked;
    for (0..5) |rank| {
        var mx: f32 = -std.math.inf(f32);
        var arg: usize = 0;
        ...
        try stdout.print("  {d}. id={d:<6} logit={d:.2}  {s}\n", .{ rank + 1, arg, mx, tokzr.decode(@intCast(arg)) });
        logits[arg] = -std.math.inf(f32); // exclude for next rank
    }
    try stdout.print("\ntotal time: {d:.0}s ({d} prompt tokens through 61 layers, XOR experts)\n", .{ @as(f64, @floatFromInt(timer.read())) / 1e9, prompt_len });
