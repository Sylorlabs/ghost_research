# E19c: extended warm-up curve on a 512-token single context. Does surrogate
# held-out fidelity reach the ~0.90 deployment threshold with realistic warm-up?
...
src=sys.argv[1] if len(sys.argv)>1 else "acts_T512_off8000.bin"
out=open("e19c_results.txt","w")
...
N=len(X);log(f"E19c extended warm-up: {src}, layer {L}, {N} ctx tokens, r={r}")
rng=np.random.default_rng(1);perm=rng.permutation(N);nhold=min(128,N//4);te=perm[-nhold:];pool=perm[:-nhold]
...
log(f"{'warmup':>7} {'input-proj':>11} {'full-surr':>10}")
sizes=[32,64,128,256,min(len(pool),448)]
for fit in sizes:
    if fit>len(pool):break
    tr=pool[:fit];mux=X[tr].mean(0);Xc=X-mux
    _,_,Vt=np.linalg.svd(Xc[tr],full_matrices=False);B=Vt[:min(r,fit-1)]
    ipo,full=[],[];Xrte=mux+Xc[te]@B.T@B
    for e in experts:
        W1,W3,W2=EW[e]
        ipo.append(pcos(ef(X[te],W1,W3,W2),ef(Xrte,W1,W3,W2)))
        Ytr=ef(X[tr],W1,W3,W2);muy=Ytr.mean(0);_,_,VtY=np.linalg.svd(Ytr-muy,full_matrices=False);C=VtY[:min(r,fit-1)]
        z=Xc[te]@B.T;g=np.minimum(z@(B@W1.T)+mux@W1.T,10.);u=np.clip(z@(B@W3.T)+mux@W3.T,-10.,10.);h=silu(g)*u
        full.append(pcos(ef(X[te],W1,W3,W2),muy+(h@(C@W2).T)@C))
    log(f"{fit:>7} {np.mean(ipo):>11.3f} {np.mean(full):>10.3f}")
log("\n>=0.90 at achievable warm-up => OUR EXPERT is deployable; the surrogate working set fits RAM (E19: 0.43MB/expert), so decode becomes RAM-bound.")
out.close();print("E19c DONE")
