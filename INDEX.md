...
**2026-07-18b round (Round AL): matcher-free causal relevance launched** (see `docs/research/research_round_2026_07_18b.md`): AL1 constructs relevance fingerprints from intervention effects; AL2 segments causal contexts without labels or change-point scores; AL3 independently attacks any hidden matcher equivalence. **The acceptance bar is direct: causal relevance must beat the fixed generic matcher, random, replay, shuffled history and fixed routing after surface recoding and causal-law shifts.**

**2026-07-18b update (Round AL): causal fingerprints and segmentation still reduce to supplied matching/routing** (see `docs/research/research_round_2026_07_18b.md`): AL1 reaches 19008 but fixed generic effect-vector matching ties exactly; AL2 reaches 17348 but generic last-consequence matching reaches 17578 and fixed periodic routing 19008. Both are valid negatives. AL3 is **GATE READY** infrastructure, rejecting all 16 seeded hidden matcher/identifier/routing/answer leaks while conditionally admitting response-only manifests. **The next edge must construct relevance beyond fixed vector comparison or binary consequence following, with AL3 as a fail-closed audit—not as evidence of intelligence.**

**2026-07-18c round (Round AM): higher-order causal relevance launched** (see `docs/research/research_round_2026_07_18c.md`): AM1 builds relevance from delayed intervention-response trajectories; AM2 builds intervention topology from later dependencies; AM3 rejects static matcher, supplied motif/graph, schedule, router, label, target, answer, score, and overlap leaks. **A result counts only if it beats fixed vector, last-outcome, temporal and graph controls plus random/replay/shuffled history after recoding and causal-law shifts.**
