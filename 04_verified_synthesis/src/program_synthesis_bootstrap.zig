...
// benchmark.
//
// Track 2 variant: the search engine's mutation/crossover RNG is no longer
// splitMix64. It is the Run 2 discovered program from program_synthesis_inventor.
...
pub fn main() !void {
    ...
    var csv_path: []const u8 = "results/program_synthesis_bootstrap.csv";
    ...
    try stdout.print("=== PROGRAM SYNTHESIS BOOTSTRAP ===\n", .{});
    try stdout.print("search_rng=discovered_run2_mix baseline=splitMix64\n", .{});
    ...
    while (i < iterations) : (i += 1) {
        ...
        if (pool.count > 1) {
            ...
            candidate = crossover(parent, pool.programs[other_idx], &rng);
        }
